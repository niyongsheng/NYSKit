...
- (ASAuthorizationAppleIDButton *)appleSignBtn API_AVAILABLE(ios(13.0)) {
    if (!_appleSignBtn) {
        ...
        _appleSignBtn.width = _loginBtn.width;
        _appleSignBtn.height = _loginBtn.height;
        _appleSignBtn.centerX = _loginBtn.centerX;
        _appleSignBtn.top = _loginBtn.bottom + 20;
    }
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization API_AVAILABLE(ios(13.0)) {
    if ([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]) {
        ...
        // 钥匙串的方式保存
        [NYSKeyChain save:KEYCHAIN_IDENTIFIER(@"SignInWithApple") data:appleUserId];
        // 登录请求
